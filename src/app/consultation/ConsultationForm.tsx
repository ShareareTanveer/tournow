    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
          ...
          className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[var(--brand)]" />
      </div>
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[{ value: 'VIDEO_CALL', label: '🎥 Video Call' }, { value: 'PHONE_CALL', label: '📞 Phone Call' }].map((m) => (
            <button key={m.value} type="button" onClick={() => setForm({ ...form, method: m.value })}
              className={`py-3 rounded-lg border-2 text-sm font-medium transition-colors ${form.method === m.value ? 'border-[var(--brand)] bg-[var(--brand-light)] text-[var(--brand)]' : 'border-gray-200 text-gray-600 hover:border-[var(--brand)]'}`}>
              {m.label}
            </button>
          ))}
        </div>
