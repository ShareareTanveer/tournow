    ...
    <div className="min-h-screen bg-[#fbfaf7]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-28 pb-2">
        <nav className="flex items-center gap-1 text-xs text-[#8a9691] flex-wrap">
          ...
          <Link href="/packages-from-sri-lanka/family" className="hover:text-[#101817]">Packages</Link>
          <FiArrowRight size={10} />
          {pkg.destination && (
            <>
              <Link href={`/destinations/${pkg.destination.slug}`} className="hover:text-[#101817]">{pkg.destination.name}</Link>
              <FiArrowRight size={10} />
            </>
          )}
          <span className="text-[#52615d] truncate max-w-[200px]">{pkg.title}</span>
        </nav>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-4">
        ...
        <h1 className="text-3xl sm:text-5xl font-black text-[#101817] mb-3 leading-tight">{pkg.title}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6 items-start">
          ...
          <div className="lg:sticky lg:top-20 space-y-4">
            ...
            <div className="bg-white rounded-lg shadow-[0_12px_40px_rgba(16,24,23,0.10)] border border-[#e5e8e4] overflow-hidden">
              ...
              <div className="p-5 border-b border-[#edf0ed]">
                ...
                <div className="flex items-end gap-2 flex-wrap">
                  ...
                  {pkg.oldPrice && (
                    <span className="text-sm text-[#8a9691] line-through mb-0.5">LKR {pkg.oldPrice?.toLocaleString()}</span>
                  )}
                </div>
                {pkg.paxType && <p className="text-xs text-[#8a9691] mt-0.5">{pkg.paxType}</p>}
            ...
            {(pkg.hostLanguage?.length > 0 || pkg.audioGuideLanguage?.length > 0) && (
              <div className="bg-white rounded-lg p-5 shadow-sm border border-[#e5e8e4] space-y-3">
                <p className="text-sm font-bold text-[#101817] flex items-center gap-2">
                  <FiGlobe size={14} style={{ color: 'var(--teal)' }} /> Languages
                </p>
                {pkg.hostLanguage?.length > 0 && (
                  <div>
                    <p className="text-xs text-[#8a9691] mb-1">Host / Guide</p>
                ...
                {pkg.audioGuideLanguage?.length > 0 && (
                  <div>
                    <p className="text-xs text-[#8a9691] mb-1">Audio Guide</p>
                    <div className="flex flex-wrap gap-1">
