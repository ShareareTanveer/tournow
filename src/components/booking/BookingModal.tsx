...
import { FiX, FiCalendar, FiUsers, FiCheckCircle, FiChevronRight, FiUpload, FiInfo } from 'react-icons/fi'
...
type Step = 'dates' | 'options' | 'summary' | 'done'
...
export default function BookingModal({ open, onClose, target }: Props) {
  ...
  if (!customer && !authOpen) {
    return (
      <AuthModal
        open={true}
        onClose={onClose}
        onSuccess={() => {}} // stays open, will re-render with customer
        initialTab="login"
      />
    )
  }
  ...
  // Price calculation
  const basePrice = form.roomType === 'TWIN' && target.priceTwin ? target.priceTwin : target.price
  const adultTotal = basePrice * form.paxAdult
  const childTotal = (target.priceChild ?? basePrice * 0.7) * form.paxChild
  ...
  const grandTotal = adultTotal + childTotal + extraTotal + optionsTotal
  ...
  const canGoToOptions = form.travelDate && form.paxAdult >= 1
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[92vh] mt-12">
          ...
          {step !== 'done' && (
            <div className="flex px-6 pt-3 pb-1 gap-1 shrink-0">
              {(['dates', 'options', 'summary'] as Step[]).map((s, i) => (
                <div key={s} className="flex items-center gap-1 flex-1">
                  <div className={`h-1 flex-1 rounded-full transition-colors ${
                    ['dates', 'options', 'summary'].indexOf(step) >= i ? 'bg-orange-400' : 'bg-gray-200'
                  }`} />
                </div>
              ))}
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {ROOM_TYPES.map(rt => (
                      <button key={rt.value} type="button"
                        ...
                        className={`text-sm py-2.5 px-3 rounded-xl border-2 font-medium transition-colors text-left ${
                          form.roomType === rt.value
                            ? 'border-orange-400 bg-orange-50 text-orange-600'
                            : 'border-gray-200 text-gray-600 hover:border-gray-300'
                        }`}>
                        {rt.label}
                        {rt.value === 'TWIN' && target.priceTwin && (
                          <span className="block text-[10px] text-green-600 font-semibold">
                            LKR {target.priceTwin.toLocaleString()}/person
                          </span>
                        )}
                      </button>
                    ))}
              ...
              <>
                <Section title="Booking Summary">
                  <div className="space-y-2 text-sm">
                    ...
                    <Row label="Travellers" value={`${form.paxAdult} Adult${form.paxAdult > 1 ? 's' : ''}${form.paxChild > 0 ? `, ${form.paxChild} Child` : ''}${form.paxInfant > 0 ? `, ${form.paxInfant} Infant` : ''}`} />
                    <Row label="Room Type" value={ROOM_TYPES.find(r => r.value === form.roomType)?.label ?? form.roomType} />
                    {form.extraNights > 0 && <Row label="Extra Nights" value={`${form.extraNights} night${form.extraNights > 1 ? 's' : ''}`} />}
                  </div>
                </Section>

                <Section title="Price Breakdown">
                  <div className="space-y-1.5 text-sm">
                    <Row label={`Base (${form.paxAdult} adult${form.paxAdult > 1 ? 's' : ''})`} value={`LKR ${adultTotal.toLocaleString()}`} />
                    ...
                    <div className="border-t border-gray-100 pt-2 mt-2 flex justify-between font-bold text-gray-900">
                      <span>Total</span>
                      <span className="text-lg" style={{ color: 'var(--brand)' }}>LKR {grandTotal.toLocaleString()}</span>
                    </div>
                  </div>
                </Section>
                ...
                <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl flex gap-2">
                  ...
                  <p className="text-xs text-amber-700">
                    After submitting, our team will call you to finalise dates, payment, and send a confirmation with all details.
                  </p>
                </div>
              </>
            )}
            ...
            {step === 'done' && (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-gray-600 mb-6 max-w-xs mx-auto">
                  Our team will contact you shortly on <strong>{customer?.email}</strong> to confirm your booking and payment details.
                </p>
                <div className="space-y-2">
                  <a href="/my/bookings"
          ...
          {step !== 'done' && (
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between shrink-0 bg-white">
              {step !== 'dates' ? (
                <button onClick={() => setStep(s => s === 'summary' ? 'options' : 'dates')}
              ...
              <div className="flex items-center gap-3">
                {step !== 'summary' && (
                  <div className="text-right">
                    ...
                    <p className="font-black text-base" style={{ color: 'var(--brand)' }}>LKR {grandTotal.toLocaleString()}</p>
                  </div>
                )}
                {step === 'dates' && (
