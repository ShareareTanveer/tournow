...
export default function WhyUsSection() {
  return (
    <section className="py-24 bg-[#0f172a] relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full bg-[#f59e0b]/5 blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full bg-[#0d9488]/5 blur-3xl pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 relative">
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 bg-white/5 text-[#f59e0b] text-xs font-bold tracking-widest uppercase px-4 py-2 rounded-full border border-white/10 mb-4">
            Why Metro Voyage
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {PERKS.map(({ icon: Icon, title, desc, color }) => (
            <div
              key={title}
              className="group flex gap-4 p-6 rounded-2xl bg-white/5 hover:bg-white/8 border border-white/8 hover:border-[#f59e0b]/30 transition-all duration-300 hover:-translate-y-1"
            >
              ...
              <div>
                <h3 className="font-bold text-white mb-1.5 group-hover:text-[#f59e0b] transition-colors">{title}</h3>
